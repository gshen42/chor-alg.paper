...
A signature \AgdaRecord{Sig} specifies the equipped operations of an algebra, which includes a type \AgdaField{Op} of operations and a function \AgdaField{Arity} giving the number of arguments (represented as the cardinality of a type) of each operation:
%
...
In programming, we can regard that a kind of effectful computation gives rise to an algebra with the allowed effects being the signature, the result of an effect being the arity of the operation, and a carrier set of such an algebra being a functional model of the effectful computation.
...
In programming, \AgdaFunction{interp} forms the foundations of effect handlers.
%
